...
public class Day9 extends Day {
	...
	private static List<Point> getLowestPoints(@NotNull HashMap<Point, Integer> heatMap, int heightOfMap, int lengthOfMap) {
		return heatMap.entrySet()
		              ...
		              .map(pointIntegerEntry ->
				              new Point(
						              pointIntegerEntry.getKey().x(),
						              pointIntegerEntry.getKey().y))
		              .collect(Collectors.toList());
	}

	public static @NotNull HashSet<HashSet<Point>> getBasins(HashMap<Point, Integer> heatMap,
	                                                         int heightOfMap,
	                                                         int lengthOfMap,
	                                                         int edgeValue) {
	...
	public String resultPartOne() {
		return getLowestPoints(heatMap, this.heightOfMap, this.lengthOfMap)
				.stream()
	...
	public String resultPartTwo() {
		var list = getBasins(heatMap, this.heightOfMap, this.lengthOfMap, 9)
				...
				.toList();
		return list.get(list.size() - 3).size() *
				list.get(list.size() - 2).size() *
