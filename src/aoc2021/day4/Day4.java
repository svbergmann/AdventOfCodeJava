...
public class Day4 extends Day {
	...
	public String resultPartTwo() {
		...
		for (var i : this.drawnNumbers) {
			...
			if (this.boards.size() == 1 && this.boards.get(0).won()) {
				return this.boards.get(0).sumUnmarkedNumbers() + " * " + i + " = " + this.boards.get(0).sumUnmarkedNumbers() * i + "";
			}
	...
	private void init(boolean example) {
		this.drawnNumbers = example ? new ArrayList<>(Arrays.stream(this.example.get(0).split(",")).map(Integer::parseInt).toList()) : new ArrayList<>(Arrays.stream(this.input.get(0).split(",")).map(Integer::parseInt).toList());
		...
		for (var i = 2; i < size; i += 6) {
			var tmp = new int[5][5];
			for (var j = 0; j < 5; j++) {
				var split = example ? Arrays.stream(this.example.get(i + j).split("\s")).filter(s -> !s.isEmpty()).toList() : Arrays.stream(this.input.get(i + j).split("\s")).filter(s -> !s.isEmpty()).toList();
				for (int k = 0; k < 5; k++) {
