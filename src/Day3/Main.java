...
public class Main {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        String string = ReadAndSaveUtils.returnFileContent("src/Day3/InputWires");
    ...
    private static void generateWires(String[] wires, WirePiece[] wirePieces) {
        for (int i = 0; i < wires.length; i++) {
            if (i == 0) {
                wirePieces[i] = new WirePiece(new Position(0, 0), wires[i].charAt(0), Integer.parseInt(wires[i].substring(1)));
            } else {
                wirePieces[i] = new WirePiece(wirePieces[i - 1].getPositions()[wirePieces[i - 1].getPositions().length - 1], wires[i].charAt(0), Integer.parseInt(wires[i].substring(1)));
            }
        }
    }
